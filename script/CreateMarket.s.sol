...
contract CreateMarketScript is Script {
    function run() external {
        ...
        IGTXOracleServiceManager.Source[] memory wethSources =
            new IGTXOracleServiceManager.Source[](2);
        wethSources[0] =
        ...
        address wethMarket =
            Router(router).createMarket(wethAddress, shortToken, "WETH/USDT", wethSources);
        console.log("WETH_USDC_MARKET_ADDRESS=%s", address(wethMarket));

        // Create WBTC/USDC market
        IGTXOracleServiceManager.Source[] memory wbtcSources =
            new IGTXOracleServiceManager.Source[](2);
        wbtcSources[0] =
            IGTXOracleServiceManager.Source({name: "binance", identifier: "WBTCUSDT", network: ""});
        wbtcSources[1] =
            IGTXOracleServiceManager.Source({name: "okx", identifier: "BTC-USDT", network: ""});
        address wbtcMarket =
            Router(router).createMarket(wbtcAddress, shortToken, "WBTC/USDT", wbtcSources);
        console.log("WBTC_USDC_MARKET_ADDRESS=%s", address(wbtcMarket));

        // Create PEPE/USDC market
        IGTXOracleServiceManager.Source[] memory pepeSources =
            new IGTXOracleServiceManager.Source[](2);
        pepeSources[0] =
            IGTXOracleServiceManager.Source({name: "binance", identifier: "PEPEUSDT", network: ""});
        pepeSources[1] =
            IGTXOracleServiceManager.Source({name: "okx", identifier: "PEPE-USDT", network: ""});
        address pepeMarket =
            Router(router).createMarket(pepeAddress, shortToken, "PEPE/USDT", pepeSources);
        console.log("PEPE_USDC_MARKET_ADDRESS=%s", address(pepeMarket));

        // Create TRUMP/USDC market
        IGTXOracleServiceManager.Source[] memory trumpSources =
            new IGTXOracleServiceManager.Source[](2);
        trumpSources[0] =
            IGTXOracleServiceManager.Source({name: "binance", identifier: "TRUMPUSDT", network: ""});
        trumpSources[1] =
            IGTXOracleServiceManager.Source({name: "okx", identifier: "TRUMP-USDT", network: ""});
        address trumpMarket =
            Router(router).createMarket(trumpAddress, shortToken, "TRUMP/USDT", trumpSources);
        console.log("TRUMP_USDC_MARKET_ADDRESS=%s", address(trumpMarket));

        // Create DOGE/USDC market
        IGTXOracleServiceManager.Source[] memory dogeSources =
            new IGTXOracleServiceManager.Source[](2);
        dogeSources[0] =
            IGTXOracleServiceManager.Source({name: "binance", identifier: "DOGEUSDT", network: ""});
        dogeSources[1] =
            IGTXOracleServiceManager.Source({name: "okx", identifier: "DOGE-USDT", network: ""});
        address dogeMarket =
            Router(router).createMarket(dogeAddress, shortToken, "DOGE/USDT", dogeSources);
        console.log("DOGE_USDC_MARKET_ADDRESS=%s", address(dogeMarket));

        // Create LINK/USDC market
        IGTXOracleServiceManager.Source[] memory linkSources =
            new IGTXOracleServiceManager.Source[](2);
        linkSources[0] =
            IGTXOracleServiceManager.Source({name: "binance", identifier: "LINKUSDT", network: ""});
        linkSources[1] =
            IGTXOracleServiceManager.Source({name: "okx", identifier: "LINK-USDT", network: ""});
        address linkMarket =
            Router(router).createMarket(linkAddress, shortToken, "LINK/USDT", linkSources);
        console.log("LINK_USDC_MARKET_ADDRESS=%s", address(linkMarket));
        ...
        //     wethAddress, shortToken, "WETH/USDT", wethSources
        // );
