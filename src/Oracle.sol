...
contract Oracle is Ownable, ReentrancyGuard {
    ...
    constructor(uint256 _minBlockInterval, uint256 _maxBlockInterval) Ownable(msg.sender) {
    ...
    function validateBlockInterval(
        uint256 blockNumber,
    ...
    function validateSignature(address token, SignedPrice calldata priceData) internal view {
        bytes32 messageHash = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                keccak256(
                    abi.encodePacked(
                        token, priceData.price, priceData.timestamp, priceData.blockNumber
                    )
                )
            )
        );
